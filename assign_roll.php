...
if(!empty($input_data))
{
  ...
  $rol="SELECT roll_id FROM roles WHERE roll_name='$r_name'";
    $rol1=$conn->query($rol);
   if($rol1->num_rows>0){
   ...
   $data= $rol3['roll_id'];
        ...
        $response['message'] = 'Not found this roll.';
        $response['status']  = 400;
        print_r(json_encode($response));
        http_response_code(400);
    ...
    $ac="INSERT INTO user_roles (user_id,roll_id) VALUES ('$data1','$data')";
